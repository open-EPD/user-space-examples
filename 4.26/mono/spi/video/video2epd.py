#!/usr/bin/env python3
"""Pack a video into an .epdv frame file for the
4.26" mono PixPaper.

Every frame is stored exactly as the controller wants it in RAM, so the on-board
player only has to memcpy it out over SPI: row-major, 100 bytes per row, the
top bit of each byte being the leftmost of its eight pixels.

Bit value 1 = white, 0 = black.

examples:
  python3 video2epd.py clip.mp4 -o clip.epdv --fps 15
  python3 video2epd.py clip.mp4 -o clip.epdv --fps 15 --preview sheet.png
"""
...
DISP_W = 800
DISP_H = 480
DISP_STRIDE = DISP_W // 8               # 100 bytes per row
FRAME_BYTES = DISP_STRIDE * DISP_H      # 48000
...
def parse_args():
    ...
    p.add_argument("--fps", type=float, default=15.0,
                   help="playback frame rate (default 15, about what the "
                        "player's defaults sustain; measure yours with --free)")
...
def pack_frame(bw):
    """bw: bool array indexed [y][x], True = white, shape (480, 800).
    Returns FRAME_BYTES of controller-RAM-order data."""
    # packbits is MSB-first along the row, which is what the panel clocks out.
    # No horizontal flip: png2bit_426.py mirrors its input, but on this wiring
    # that comes out backwards -- checked against the panel.
    return np.packbits(bw, axis=1).tobytes()


def unpack_frame(data):
    """Inverse of pack_frame, for --preview."""
    return np.unpackbits(
        np.frombuffer(data, dtype=np.uint8).reshape(DISP_H, DISP_STRIDE), axis=1
    ).astype(bool)


def write_preview(pack_path, png_path, tiles=9, scale=2):
    ...
    with open(pack_path, "rb") as f:
        ...
        for i in idx:
            ...
            vis = unpack_frame(f.read(FRAME_BYTES))
...
def main():
    ...
    if args.fps > src_fps:
        # there is nothing to sample past the source's own rate, so the pack
        # would hold every frame there is but claim to play them faster --
        # i.e. silently speed the clip up
        sys.exit(f"--fps {args.fps:g} is above the source's {src_fps:.2f} fps; "
                 f"the clip would play {args.fps / src_fps:.2f}x too fast")
    ...
    print(f"output: {args.output}  {dst_w}x{dst_h} @ {args.fps} fps"
    ...
    with open(args.output, "wb") as out:
        ...
        out.seek(0)
        out.write(struct.pack(HDR_FMT, EPDV_MAGIC, EPDV_VERSION, 0,
    ...
    if args.preview:
        idx = write_preview(args.output, args.preview)
