 ...
 * Date: 2026-08-11
 *
 * Video player for Raspberry Pi 5: libgpiod v2 GPIO backend.
 * Shared SPI/EPD/player logic lives in video_app.h.
 *
 * Check the pins below against your wiring before the first run.
 */
#include <gpiod.h>
#include <stdio.h>
#include <stdlib.h>

/* The header GPIO has moved between Pi 5 kernels (gpiochip4/15, gpiochip0 on
 * 6.12+).  `gpiodetect` shows which one is labelled pinctrl-rp1; override with
 * make EXTRA_CFLAGS='-DEPD_GPIO_CHIP=\"gpiochip0\"'. */
#ifndef EPD_GPIO_CHIP
